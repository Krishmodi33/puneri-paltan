...
const SinglePlayer = () => {
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen bg-black flex items-center justify-center">
        <h1 className="text-4xl font-bold text-white">Loading Player...</h1>
      </div>
    );
  }
  ...
  if (!player) {
    return (
      <div className="min-h-screen bg-black flex items-center justify-center">
        <h1 className="text-white text-3xl">No Player Found</h1>
      </div>
    );
  }

  const imageSrc = player.profile_image || player.full_image || player.image;

  const otherPlayers = allPlayers.filter(
    (p) =>
      String(p.player_id || p.id) !== String(player.player_id || player.id),
  );
  return (
    <>
      ...
      "
        ...
        "
            >
              ...
              <StatCard value={player.super_10s || "0"} label="SUPER 10s" />
